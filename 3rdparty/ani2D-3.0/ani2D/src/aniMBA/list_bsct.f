...
C ======================================================================
      Implicit none
      Integer L1E(2,*), L2E(*), LHol(*), nE, nL2, iCntl(4)
...
C     Chan  = iCntl(4)
      ...
      If(L1E(1,iE).eq.0.and.L1E(2,iE).eq.0) Then
         Call errMesMBA(5101, 'lstUpd', 'no element in the list')
      End if

      Call fpcheck(qiE, flagFP)

      If(flagFP) Then
         Call errMesMBA(5111, 'lstUpd', 'bad input value(NAN/INF)')
      End if
...
c  ...    the last one may be just positive !!!
         ...
         Else
              If(L2E(nL2).ne.0) Then
                 Call errMesMBA(5102, 'lstUpd', 
     &                         'L2E(nL2) must be 0 here')
              End if
              nL2 = nL2 - 1
         End if
      End if
...
C     Chan  = iCntl(4)
      ...
      If(L1E(1,iE).eq.0.and.L1E(2,iE).eq.0) Then
         Call errMesMBA(5103, 'lstDel', 'no element in the list')
      End if
